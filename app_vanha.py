...
<style>
  ...
  /* ── File uploader: clean slate ─────────────────────────────────────────────
     Let Streamlit render its natural browse-button and file-chip; we only
     control the box appearance and the button's colour scheme. */
  ...
  /* Browse-files button: muted style, red on hover */
  ...
  /* Limit text (200MB, types) */
  [data-testid="stFileUploaderDropzone"] small {
    color:         var(--muted) !important;
    font-size:     0.68rem !important;
  }

  /* ── Custom compositing spinner ──────────────────────────────────────────── */
  ...
  .cas-ring {
    ...
    border-radius: 50%;
    animation:     cas-spin 0.75s linear infinite;
  }
  ...
  /* Sidebar header area: Streamlit renders a fixed-height header block that holds
     the collapse button. Collapsing it removes the dead space above the logo. */
  ...
  /* Sidebar top space: zero out padding on content containers (logo inline style
     provides the desired 1 rem breathing room) */
  ...
  section[data-testid="stSidebar"] [data-testid="stVerticalBlock"] {
    ...
    padding-top: 0 !important;
    gap:         0.65rem !important;  /* ~15 % tighter than Streamlit default */
  }

  /* Sidebar spacing: only tighten dividers and caption */
  ...
  [data-baseweb="checkbox"] [data-checked="true"] svg {
    fill:  #505045 !important;
    color: #505045 !important;
  }

  /* File-uploader drag-over overlay */
  [data-testid="stFileUploaderDropzoneDragOver"],
  [data-testid="stFileUploaderDropzone"][data-drag-over="true"],
  [data-testid="stFileUploaderDropzone"]:focus-within {
    background-color: rgba(217, 214, 200, 0.25) !important;
    border-color:     #D9D6C8 !important;
  }

  /* BaseWeb tag / multiselect chip */
  [data-baseweb="tag"] {
    background-color: #D9D6C8 !important;
    color:            #505045 !important;
  }
  [data-baseweb="tag"] span { color: #505045 !important; }

  /* st.code blocks (anchor debug coordinates) */
  [data-testid="stCode"],
  .stCode > div {
    background-color: var(--bg-card)    !important;
    border:           1px solid var(--border) !important;
  ...
  /* Any remaining BaseWeb focus ring that shows blue */
  [data-baseweb] *:focus-visible {
    outline-color: #D9D6C8 !important;
    box-shadow: 0 0 0 3px rgba(217,214,200,0.45) !important;
  }
...
def render_header() -> None:
    ...
    st.markdown(
        ...
        f'      <span style="font-family:\'Gotham\',\'Barlow\',sans-serif;font-weight:700;">Cast-a-shadow</span>'
        f'      <span style="font-family:\'Chevin Pro\',\'Barlow\',sans-serif;font-weight:400;font-size:1rem;'
...
def main() -> None:
    ...
    _status = st.empty()
    _status.markdown(
        '<div class="cas-status"><span class="cas-ring"></span>Compositing…</div>',
        unsafe_allow_html=True,
    )
    result = composite(
            product_img,
